...
Also, there are folders specific to render pipelines: Built-In RP, URP, and HDRP. They exist in the folders MudBun/Examples and MudBun/Shader/Render.
Only import the folders that are specific to the current render pipeline and leave out the rest. It wouldn't matter in play mode in editor, but can cause errors when building executables
DO NOT move the MudBun root folder away from the Assets root folder, as this is a necessary trade-off for not having to force users to place custom shaders in the same folder as MudBun's built-in shaders.
...
Version 1.3.30
 ...
 - Fix rendeer's Always Draw Gizmos option not drawing brush gizmos.
